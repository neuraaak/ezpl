...
class UserEnvManager:
    ...
    def _save_env_file(self, env_vars: dict[str, str]) -> None:
        ...
        Raises:
            OSError: If writing to the .env file fails
        """
        self.env_file.parent.mkdir(parents=True, exist_ok=True)

        # Keep only managed variables to ensure removals are persisted
        managed_vars = {
            key: value for key, value in env_vars.items() if key.startswith("EZPL_")
        }
        ...
        with open(self.env_file, "w", encoding="utf-8") as f:
            ...
            for key, value in sorted(managed_vars.items()):
                f.write(f"{key}={value}\n")
